...
public class INSE_Server {
	...
	public static void main(String[] args){
		...
		try {
			setupLogging();
			Department inse = Department.INSE;
			skelton = new EnrollmentImpl(inse.toString());
			Endpoint endpoint = Endpoint.publish("http://localhost:"+inse.getWebServicePort()+"/" + inse.toString().toLowerCase(), skelton);
		} catch (Exception e) {
			e.printStackTrace();
		}
