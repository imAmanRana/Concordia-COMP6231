...
public class SOEN_Server {
	...
	public static void main(String[] args) {
		try {
			setupLogging();
			Department soen = Department.SOEN;
			skelton = new EnrollmentImpl(soen.toString());
			Endpoint endpoint = Endpoint.publish("http://localhost:"+soen.getWebServicePort()+"/" + soen.toString().toLowerCase(), skelton);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static void setupLogging() throws IOException {
		...
		if (!files.exists())
			files.mkdirs();
		files = new File(Constants.SERVER_LOG_DIRECTORY + "SOEN_Server.log");
		if (!files.exists())
			files.createNewFile();
